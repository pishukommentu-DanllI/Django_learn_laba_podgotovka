...
class SingUp(CreateView):
    ...
    template_name = 'app/SingUp.html'  # ссылка на шаблон (где хранится это html)
...
def create_Dishes():
    if Dishes.objects.all().count() == 0:
        Dishes.objects.create(name='soup', compound='Carrot, Potatoes, Chicken')
        Dishes.objects.create(name='pie', compound='Milk, Flour, Chocolate')
        Dishes.objects.create(name='meat', compound='Meat, Spices')
        Dishes.objects.create(name='Caesar salad', compound='vinegar, croutons, Chinese cabbage')
        Dishes.objects.create(name='cheesecake', compound='shortbread cookies, butter, cream cheese')


def create_Hall():
    if Hall.objects.all().count() == 0:
        Hall.objects.create(name='Hall_small', info='Small hall with 30 seats. Specifically suitable for lamp gatherings with friends')
        Hall.objects.create(name='Hall_midle', info='average hall for 60 seats. Great for big family celebrations')
        Hall.objects.create(name='Hall_big', info='Large hall for 110 seats. Suitable for important negotiations')
